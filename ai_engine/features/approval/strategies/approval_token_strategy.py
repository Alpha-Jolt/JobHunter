...
def generate_token(
    variant_id: str, user_id: str, action: ApprovalStatus, key_id: str, secret: str
) -> str:
    """Generate a signed approval token.

    Token format: ``{version}.{key_id}.{payload_b64}.{signature}``
    ...
    Args:
        ...
        key_id: ID of the active HMAC key.
    ...
    return f"{_TOKEN_VERSION}.{key_id}.{payload_b64}.{sig}"


def validate_token(token: str, keyring: dict[str, str]) -> dict:
    """Validate and decode an approval token.

    Args:
        token: Token string from the approval link.
        keyring: Dict of {key_id: raw_secret}.
    ...
    _, key_id, payload_b64, provided_sig = parts
    if key_id not in keyring:
        raise ValueError(f"Unknown key_id '{key_id}' — key may have been retired.")
